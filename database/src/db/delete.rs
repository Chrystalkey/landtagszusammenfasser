...
use openapi::apis::default::{AsDeleteResponse, VorgangDeleteResponse};
use uuid::Uuid;
...
pub async fn delete_ass_by_api_id(api_id: Uuid, server: &LTZFServer) -> Result<AsDeleteResponse> {
    let thing = sqlx::query!(
        "DELETE FROM ausschusssitzung WHERE api_id = $1 RETURNING id",
        api_id
    )
    .fetch_optional(&server.sqlx_db)
    .await?;
