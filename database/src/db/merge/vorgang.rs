...
vorgangstyp.value = $5 AND
    ...
    Ok(match result.len() {
        ...
        1 => MergeState::OneMatch(result[0].vg_id),
        _ => {
            ...
            MergeState::AmbiguousMatch(
                result.iter().map(|x|x.vg_id).collect()
            )
        }
    })
}
...
pub async fn run_integration(model: &models::Vorgang, server: &LTZFServer) -> Result<()> {
    ...
    match candidates {
        ...
        MergeState::OneMatch(one) => {
            let api_id = sqlx::query!("SELECT api_id FROM vorgang WHERE vg_id=$1", one)
