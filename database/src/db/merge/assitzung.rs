...
pub async fn ass_merge_candidates(
    ...
    let cands = sqlx::query!(
        ...
        ",
        model.api_id,
        model.ausschuss.parlament.to_string(),
        model.ausschuss.name,
        model.ausschuss.wahlperiode as i32,
        model.termin
    )
    .map(|r| r.id)
    .fetch_all(&mut **tx)
...
pub async fn execute_merge_ass(
    ...
    sqlx::query!(
        "INSERT INTO rel_ass_tops(top_id, ass_id)
    SELECT blub, $1 FROM UNNEST($2::int4[]) blub",
        db_id,
        &tids[..]
    )
    .execute(&mut **tx)
...
pub async fn run_integration(model: &models::Ausschusssitzung, server: &LTZFServer) -> Result<()> {
    ...
    match candidates {
        ...
        MergeState::AmbiguousMatch(many) => {
            tracing::warn!(
                "Ambiguous matches for Ausschusssitzung with api_id: {:?}",
                model.api_id
            );
